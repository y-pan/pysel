...
def loginL(driver, username, password, entryweb):  # l
    driver.get(entryweb)
    driver.find_element_by_xpath(
        "//section[@id='find-your-way']//a[contains(text(),'eLearning')]").click()
    driver.find_element_by_xpath(
        "//a[contains(text(), 'Lynda.com')][1]").click()
    driver.find_element_by_xpath(
        "//div[@class='record-detail']//a[contains(@class, 'access-online')]").click()
    driver.find_element_by_xpath(
        "//input[@id='card-number']").send_keys(username)
...
def scrollDownL(driver, num):
    ys = driver.execute_script(
        f"var ele=document.getElementsByClassName('course-toc toc-container')[0]; y0=ele.scrollTop; ele.scrollBy(0,{num}); y1=ele.scrollTop; return [y0, y1];")
    var.SHOW_MSG and print(f"Scroll down [from, to]: {ys}")


def scrollDownS(driver, num):
    ys = driver.execute_script(
        f"var y0=window.pageYOffset; window.window.scrollTo(0, (window.pageYOffset + {num})); return [y0, window.pageYOffset];")
    var.SHOW_MSG and print(f"Scroll down [from, to]: {ys}")
